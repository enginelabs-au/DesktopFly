...
MALECNS_FIXTURE = REPO_ROOT / "backend" / "fixtures" / "malecns-motor-circuit-subset.json"


def test_runtime_loader_uses_malecns_subset_when_enabled():
...
def test_malecns_fixture_is_not_synthetic_ids():
    tables = json.loads(MALECNS_FIXTURE.read_text(encoding="utf-8"))
